...
if (window == top) {
    chrome.extension.onMessage.addListener(function (profileTypes, sender, sendResponse) {
        sendResponse(getProfileType(profileTypes));
    });
}
...
var getProfileType = function (profileTypes) {
    ...
    for (var i = 0; i < profileTypes.length; i++) {
        if (hostname.indexOf(profileTypes[i].hostname) >= 0) {
            return profileTypes[i].name;
        }
    }
}
